...
class Policy(torch.nn.Module):  #Sub-class of NN PyTorch class
    """ Policy neural network and beta distribution construction"""
    ...
    def forward(self, x):
        ...
        beta_dist = Beta(action_alpha_beta[:3], action_alpha_beta[3:])
...
class Value(torch.nn.Module):
    """ Value neural network for Actor-critic of type 1"""
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, torch.nn.Linear):
                torch.nn.init.xavier_uniform_(m.weight)  # Initialization Xavier more stable
                torch.nn.init.zeros_(m.bias)
...
class Agent(object):
    ...
    def update_policy(self):
        """Optimization step of the policy (if necessary)"""
        ...
        #Algorithm: REINFORCE
        if self.type_alg == 0:
            #Compute and normalize returns (if needed)
            ...
            self.n_episode +=1
            #If the current episode is the last of the batch
            if self.n_episode==self.batch_sz:
                padded_returns = pad_sequence(self.return_list, batch_first=True)
                #Compute the dynamic baseline as the mean of returns at each timestep
                baseline = torch.mean(padded_returns, 0)
                #Compute the total loss of the batch as the sum or average of episode losses
                tot_loss = 0
        ...
        #Algorithm: Actor-Critic of type 1
        ...
        #Algorithm: Actor-Critic of type 2
    ...
    def get_action(self, state, evaluation=False):
        ...
        @return beta_dist_det detached action beta distribution
        """
        ...
        if evaluation:  # Return mean
            ...
            return action, action_log_prob, beta_dist_det  # beta_dist_det is detached
    ...
    def get_probs(self, state, action):
        ...
        @param state current state
        @param action action of interest
        @return action_log_prob probability of chosen action (joint probability of the three action values)
        """
        x = torch.from_numpy(state).float().to(self.train_device)
        out,_ = self.policy(x)
